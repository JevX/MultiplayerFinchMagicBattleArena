...
namespace DigitalRubyShared
{
    ...
    public static class FingersExampleSceneTransitionScript
    {
        ...
        private static IEnumerator TransitionCoRoutine(Camera currentCamera, Camera newCamera, Scene currentScene, Scene newScene, bool unload)
        {
            ...
            if (unload)
            {
                UnityEngine.SceneManagement.SceneManager.UnloadSceneAsync(currentScene).completed += (_op) =>
                {
        ...
        public static void PushScene(string name, bool animated = true)
        {
            ...
            AsyncOperation op = UnityEngine.SceneManagement.SceneManager.LoadSceneAsync(name, LoadSceneMode.Additive);
            if (animated)
            {
                op.completed += (_op) =>
                {
        ...
        {
            Scene currentScene = UnityEngine.SceneManagement.SceneManager.GetSceneAt(UnityEngine.SceneManagement.SceneManager.sceneCount - 1);
            if (animated)
            {
                Scene newScene = UnityEngine.SceneManagement.SceneManager.GetSceneAt(UnityEngine.SceneManagement.SceneManager.sceneCount - 2);
