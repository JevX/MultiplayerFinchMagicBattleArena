...
namespace Wand
{
    public class MagicShooterKinetic : MagicShooter, MMEventListener<MMGameEvent>
    {
        ...
        IEnumerator GuideObjectToWand(float shiftDistance)
        {
            ...
            while (pos < 1)
            {
                carriedObject.transform.position = Vector3.Lerp(objectOriginalPosition, MyWand.wandEndPosition.position + (MyWand.wandEndPosition.forward * shiftDistance), pos);
                pos += Time.deltaTime * lerpObjectGuideSpeed;
                yield return null;
            }
            carriedObject.transform.parent = MyWand.wandEndPosition;
            carriedObject.transform.position = MyWand.wandEndPosition.position + (MyWand.wandEndPosition.forward * shiftDistance);
            yield return null;
        }
        ...
        public void CarryTargetedObject(Collider objectToCarry)
        {
            ...
            Vector3 closestPoint = carriedObject.ClosestPointOnBounds(MyWand.wandEndPosition.position);
                ...
                if (myShooter.hitsArray.Length > 0)
                {
                    ...
                    foreach (RaycastHit hittedObject in myShooter.hitsArray)
                    {
                        if (hittedObject.rigidbody != null)
                        {
                            ...
                            {
                                hittedObject.rigidbody.AddForce(-MyWand.wandEndPosition.forward * kineticForce, ForceMode.Force);
                            }
